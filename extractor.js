...
var btn_extract = document.getElementById('btn_extract') ;
var cors = "http://allow-any-origin.appspot.com/" ;
var func_def, helperobj ;
var head = document.getElementsByTagName('head')[0] ;
var link = document.createElement('link') ;
var ipc = require('ipc');

btn_extract.addEventListener('click', function () {
    initiate();
});
...
function loading_spinner () {
    ...
    link.rel = 'stylesheet' ;
    link.href = 'spinner.css' ;
    link.type = 'text/css' ;
    head.appendChild(link) ;
}

function extract_URL(video_id) {

    var video_webpage, embed_webpage, age_gate, url_data, video_info_url, param_arr, video_info_webpage, url_list = [];
            ...
            setTimeout(
                function(){
                                ...
                                video_info_url = "http://www.youtube.com/get_video_info?" + url_data ;
                                {
                                    ...
                                    xhr.onload = function(){
                                        if(xhr.responseText != undefined)
                                            ...
                                            alert("Webpage with url = " + video_info_url + "couldn't be loaded!") ;
                    ...
                    try{
                        ...
                        param_arr = ytplayer_config.args ;
                            ...
                            video_url += "&signature" + url_data.sig ;
                                    ...
                                    if ((jsplayer_json == undefined) && (age_gate == false))
                                    {
                                        if (embed_webpage == undefined){
                                            jsplayer_json = load_embed_page(jsplayer_json, embed_webpage);
                                        }
                                    }
                    ...
                    for (var u in url_list){
                        if(url_list[u] != undefined)
                            urlist.innerHTML += "<a href=" + url_list[u] + ">" + itag_values["_" + u] + "</a><br>" ;
                    }
...
function load_embed_page(jsplayer_json, embed_webpage) {
...
function decrypt_s (encrypted_sig, player_url, video_id) {
	...
	try{
		extract_signature_function(encrypted_sig, player_url, video_id) ;
...
function set_YTPlayer (param_arr) {
    ...
    var anim = document.getElementById('loading_anim');
